...
The server supports Google Gemini through the Generate Content REST API and OpenAI through the Responses API. Configure `AI_PROVIDER`, `AI_MODEL` and the matching `GEMINI_API_KEY` or `OPENAI_API_KEY` in an environment variable or Streamlit secrets. The key is never displayed in the interface and must never be committed to GitHub.
